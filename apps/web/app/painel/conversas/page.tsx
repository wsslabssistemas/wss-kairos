...
export default async function ConversasPage({
  ...
  return (
    <main>
      ...
      {/* ⚠ TRES ABAS, PELA MESMA REGRA DA AUTOMACAO: frequencia de uso, nao
          assunto. "Conversas" e o trabalho — quem escreveu e espera resposta —
          e vem por padrao. "Entregas" e conferencia. "Historico" e o log cru,
          que quase ninguem abre e que nao pode empurrar o trabalho para baixo.
          A tela tinha os tres empilhados, e a lista de 124 mensagens ficava
          entre a pessoa e a proxima conversa. */}
      <Abas
        abas={[
          {
            id: "conversas",
                          ...
                          <ul style={{ listStyle: "none", padding: 0, margin: "0 0 12px" }}>
                            {[...(((fio as Fio[] | null) ?? []))].reverse().map((m) => (
                              ...
                              >
                                ...
                                {/* ⚠ O ARQUIVO DEIXOU DE SER UMA FRASE MORTA. Antes ficava
                                    "(documento recebido — abra no WhatsApp)" e não existia
          ...
          {falhas > 0 && (
            <div className="card mt-24" style={{ borderColor: "var(--danger)" }}>
              ...
              {/* ⚠ AGRUPADO ANTES DA LISTA. Quatorze linhas parecidas escondem que
                  sao tres problemas distintos — e que um deles exige reacao hoje. */}
              <div className="row wrap" style={{ gap: 8, marginTop: 12 }}>
                {agruparFalhas(fs.map((f) => f.delivery_error)).map((g) => (
                  <span
                    key={g.leitura.tipo}
                    className={g.leitura.grave ? "badge badge-danger" : "badge"}
                    style={{ whiteSpace: "normal", textAlign: "left" }}
                  >
                    <strong>{g.quantas}</strong>&nbsp;· {g.leitura.resumo}
                  </span>
                ))}
              </div>
              <ul style={{ listStyle: "none", padding: 0, margin: "12px 0 0" }}>
                {fs.map((f) => (
                  <li key={f.id} style={{ padding: "10px 0", borderTop: "1px solid var(--border)" }}>
                    ...
                    {/* ⚠ EM PORTUGUES, COM O QUE FAZER — e o texto cru da Meta ao
                        lado. Antes aparecia so "Message undeliverable" e "User's
                        number is part of an experiment" para uma recepcionista
                        brasileira: duas frases parecidas que sao dois problemas
                        diferentes. Traduzir ACRESCENTA; nunca esconde, porque quem
                        for abrir chamado na Meta precisa do texto exato. */}
                    {(() => {
                      const l = lerFalha(f.delivery_error);
                      return (
                        <>
                          <p style={{ fontSize: 13, margin: "4px 0 0", color: l.grave ? "var(--danger)" : "var(--text-dim)" }}>
                            {l.grave ? "⚠ " : ""}{l.resumo}
                          </p>
                          {l.acao && (
                            <p className="text-faint" style={{ fontSize: 12, margin: "2px 0 0" }}>{l.acao}</p>
                          )}
                          {f.delivery_error && (
                            <p className="text-faint" style={{ fontSize: 11, margin: "2px 0 0", opacity: 0.75 }}>
                              <code>{f.delivery_error}</code>
                            </p>
                          )}
                        </>
                      );
